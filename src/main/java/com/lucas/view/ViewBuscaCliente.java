 ...
 * @author Juliano
 */
public class ViewBuscaCliente extends javax.swing.JDialog {

    JInternalFrame parent;

    public ViewBuscaCliente(JInternalFrame parent) {
        super();
        this.parent = parent;
        initComponents();
        carregarGrade();    
    ...
    private void carregarGrade() {
        try {
            ...
            ctm.setDados(clienteDAO.findCliente());
        } catch (Exception ex) {
    ...
    private void tbGradeMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbGradeMouseClicked
        if (evt.getClickCount() == 2) {
           ...
           if(parent instanceof ViewRelatorioCliente) {
                ViewRelatorioCliente rlv = (ViewRelatorioCliente) parent;
                rlv.setCliente(cliente);
                dispose();
            }
            else {
